 ...
 */
class phpspa{static get _events(){return{beforeload:[],load:[]}}static navigate(t,e="push"){(async()=>{phpspa.emit("beforeload",{route:t});function o(o){"string"!=typeof o?.title&&"number"!=typeof o?.title||(document.title=o.title);let n=document.getElementById(o?.targetID)??document.getElementById(history.state?.targetID)??document.body;n.innerHTML=o?.content??o;const a={url:t?.href??t,title:o?.title??document.title,targetID:o?.targetID??n.id,content:o?.content??o};"push"===e?history.pushState(a,a.title,t):"replace"===e&&history.replaceState(a,a.title,t);let r=document.getElementById(t?.hash?.substring(1));r&&scroll({top:r.offsetTop}),n.querySelectorAll("script[data-type='phpspa/script']").forEach((t=>{const e=document.createElement("script");e.textContent=t.textContent,document.head.appendChild(e).remove()}))}(await fetch(t,{method:"PHPSPA_GET",mode:"same-origin",keepalive:!0})).text().then((e=>{try{let n=JSON.parse(e);phpspa.emit("load",{route:t,success:!0,error:!1}),o(n)}catch(n){let a=e??"";phpspa.emit("load",{route:t,success:!1,error:n}),o(a)}})).catch((e=>phpspa.emit("load",{route:t,success:!1,error:e})))})()}static back(){history.back()}static forward(){history.forward()}static reload(){this.navigate(new URL(location.href),"replace")}static on(t,e){this._events[t]||(this._events[t]=[]),this._events[t].push(e)}static emit(t,e){const o=this._events[t]||[];for(const t of o)"function"==typeof t&&t(e)}}void 0===window.phpspa&&(window.phpspa=phpspa);