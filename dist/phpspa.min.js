 ...
 */
class phpspa{static _events={beforeload:[],load:[]};static navigate(t,e="push"){(async()=>{phpspa.emit("beforeload",{route:t});function n(n){"string"!=typeof n?.title&&"number"!=typeof n?.title||(document.title=n.title);let o=document.getElementById(n?.targetID)??document.getElementById(history.state?.targetID)??document.body;o.innerHTML=n?.content??n;const a={url:t?.href??t,title:n?.title??document.title,targetID:n?.targetID??o.id,content:n?.content??n};"push"===e?history.pushState(a,a.title,t):"replace"===e&&history.replaceState(a,a.title,t);let r=document.getElementById(t?.hash?.substring(1));r&&scroll({top:r.offsetTop,left:r.offsetLeft}),runInlineStyles(o),runInlineScripts(o)}(await fetch(t,{method:"PHPSPA_GET",mode:"same-origin",keepalive:!0})).text().then((e=>{try{let o=JSON.parse(e);phpspa.emit("load",{route:t,success:!0,error:!1}),n(o)}catch(o){let a=e??"";phpspa.emit("load",{route:t,success:!1,error:o}),n(a)}})).catch((e=>phpspa.emit("load",{route:t,success:!1,error:e})))})()}static back(){history.back()}static forward(){history.forward()}static reload(){this.navigate(new URL(location.href),"replace")}static on(t,e){this._events[t]||(this._events[t]=[]),this._events[t].push(e)}static emit(t,e){const n=this._events[t]||[];for(const t of n)"function"==typeof t&&t(e)}static setState(t,e){return new Promise((async(n,o)=>{let a={top:scrollY,left:scrollX};const r=new URL(location.href);phpspa.emit("beforeload",{route:r});function i(t){"string"!=typeof t?.title&&"number"!=typeof t?.title||(document.title=t.title);let e=document.getElementById(t?.targetID)??document.getElementById(history.state?.targetID)??document.body;e.innerHTML=t?.content??t;const n={url:r?.href??r,title:t?.title??document.title,targetID:t?.targetID??e.id,content:t?.content??t};history.replaceState(n,n.title,r),scroll(a),runInlineStyles(e),runInlineScripts(e)}(await fetch(r,{method:"PHPSPA_GET",body:JSON.stringify({stateKey:t,value:e}),mode:"same-origin",keepalive:!0})).text().then((t=>{try{let e=JSON.parse(t);n(),i(e)}catch(e){let n=t??"";o(e),i(n)}})).catch((t=>{o(t)}))}))}}function runInlineScripts(t){t.querySelectorAll("script[data-type='phpspa/script']").forEach((t=>{const e=document.createElement("script");e.textContent=`(function() {\n${t.textContent}\n})();`,document.head.appendChild(e).remove()}))}function runInlineStyles(t){t.querySelectorAll("style[data-type='phpspa/css']").forEach((t=>{const e=document.createElement("style");e.textContent=t.textContent,document.head.appendChild(e).remove()}))}void 0===window.phpspa&&(window.phpspa=phpspa);